...
;(function(root, factory) {
  ...
  EndUser.prototype._appendIframe = function(parentId, id, src) {
    ...
    var srcParams = '?address=' + pThis._parseURL(window.location.href).origin
  ...
  EndUser.prototype._postMessage = function(cmd, params, _resolve, _reject) {
    ...
    try {
      ...
      if (s_debug) console.log('Main page post message error: ' + e)
    }

    if (s_debug) console.log('Main page post message: ' + msg)
  ...
  EndUser.prototype._recieveMessage = function(event) {
    ...
    if (s_debug) console.log('Main page recieve message: ' + event.data)
    ...
    if (data.id == -1) {
      ...
      promises.forEach(function(promise) {
        pThis._postMessage(promise.msg.cmd, promise.msg.params, promise.resolve, promise.reject)
      })

      return
    } else if (data.id == -2) {
      var widgetEvent = data.result
      if (pThis.m_listeners[widgetEvent.type]) pThis.m_listeners[widgetEvent.type](widgetEvent)
      return
    }
   ...
   * @returns Promise<void>
   */
  EndUser.prototype.ChangeOwnCertificatesStatus = function(ccsType, revocationReason) {
    var params = Array.prototype.slice.call(arguments)
   ...
   */
  EndUser.prototype.SignHash = function(hash, asBase64String, signAlgo, signType, previousSign) {
    var params = Array.prototype.slice.call(arguments)
