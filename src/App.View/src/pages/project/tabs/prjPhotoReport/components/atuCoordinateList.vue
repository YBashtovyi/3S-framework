<template>
  <div>
    <div class="row bg-grey-2 justify-between q-mt-sm">
      <div class="text-subtitle2 text-uppercase q-pa-md">Координати проєкту</div>
      <q-btn
...
export default {
  ...
  methods: {
    ...
    onAddAtuCoordinate(item) {
      addAtuCoordinate(this.prjPhotoReportId, item).then(this.updateAtuCoordinateList)
    },

    onDeleteAtuCoordinate(order) {
      deleteAtuCoordinate(this.prjPhotoReportId, order).then(this.updateAtuCoordinateList)
    },
  ...
  computed: {
    prjPhotoReportId() {
      return this.$route.params.prjPhotoReportId
    },
  },
}
</script>