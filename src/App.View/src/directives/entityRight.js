import { getEntityAccessLevel } from './../services/securityService'

function CommentOrDisableNode(el, binding, vnode) {
  if (binding.modifiers.disableIfNo) {
...
export default {
  ...
  // ATTENTION: to work correctly the calling component should import user with rights from storage
  // Example:
  // computed: {
  ...
  bind: function(el, binding, vnode) {
    ...
    const accessLevel = getEntityAccessLevel(rights, binding.value)

    // if access = read, disable the element (for buttons) or leave as is (for journals)
    if (accessLevel === 1) {
      if (binding.modifiers.hideIfRead) {
        CommentOrDisableNode(el, binding, vnode)
      } else if (!binding.modifiers.enableIfRead) {
        el.setAttribute('disabled', true)
      }
    }
