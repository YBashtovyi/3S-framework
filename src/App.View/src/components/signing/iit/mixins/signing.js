...
export default {
  ...
  watch: {
    ...
    /*
     *  Usually when you reopen a dialog, you need to show the first tab
     */
    visibleDialog: function (value) {
      if (value) {
        this.keyValue = "file"
        this.loadIITPlugin()
      }
  ...
  methods: {
    ...
    tryLoadKeyAndSignData() {
      ...
      this.registerLoadingLabel = 'Зчитування'
      ...
      reader.onloadend = evt => {
        this.lib.readPrivateKey(file, this.password, error => {      
          if (error && error.message) {
            ...
            this.registerLoadingLabel = 'Підпис'
            this.signData()
          }
        })
      }
     ...
     */
    signData() {
      let signedData = this.lib.signData(this.dataToSign)
    ...
    signDataFileDrive() {
      ...
      window["signPrivate"](this.dataToSign, function (base64) {
     ...
     */
    openSignJson() {
      let jsonFile = decodeURIComponent(atob(this.dataToSign).split('').map(function(c) {
      ...
      x.document.open();
      x.document.write(
        `<html><head><title>Файл, що підписується</title></head><body><pre style='min-height: 30rem;line-height: 1.2;'><code>` +
         JSON.stringify(jsonParse, null, 4) +
         '</code></pre></body></html>');
      x.document.close();
    },
