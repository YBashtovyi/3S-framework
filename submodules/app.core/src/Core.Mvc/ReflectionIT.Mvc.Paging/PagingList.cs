...
namespace ReflectionIT.Mvc.Paging
{
    ...
    public class PagingList
    {
            ...
            bool readyToUse = false) where T : class
        {
            ...
            var pageCount = (int)Math.Ceiling(totalRecordCount / (double)pageSize);
            return new PagingList<T>(list, pageIndex, pageCount, totalRecordCount) { Action = action };
        }
            ...
            bool readyToUse = false) where T : class
        {
            ...
            var totalRecordCount = totalRecords ?? collection.Count();
            var pageCount = (int)Math.Ceiling(totalRecordCount / (double)pageSize);
            return new PagingList<T>(list, pageIndex, pageCount, sortExpression, defaultSortExpression, totalRecordCount) { Action = action };
        }
    }
}
