...
(select caption from cmn_enum_record  where  enum_type = 'EpisodeState'  and code = e.event_state) as event_state,
e.description,
e.doctor_id,
...
left join cmn_person as person on  pc.person_id = person.id and person.record_state<>4
left join org_employee as doc on  e.doctor_id = doc.id and doc.record_state<>4
left join cmn_person as persdoc on  doc.person_id = persdoc.id and persdoc.record_state<>4
